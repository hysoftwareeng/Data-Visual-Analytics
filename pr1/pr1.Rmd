...
scale_x_discrete("Boxplot of Runtime of Movies by Decade")

ggplot(df2, aes(reorder(Decade, Budget, mean), log(Budget), color=Decade)) +
geom_boxplot() + coord_flip() +
scale_x_discrete("Boxplot of Budget of Movies by Decade")
...
for (i in seq(1:nrow(df2))) {
  ...
  else if (is.na(df2$Year[i]) & is.na(df2$Date[i]) & df2$Date[i]) {
    df2$removal[i] = 0
  }
...
qplot(x=log(Gross), y=Runtime, data=df_q5, color=Genre, facets=Runtime_Binary~Genre) +
  stat_smooth(se = FALSE,colour="black")
...
ggplot(df5, aes(x=reorder(Released_Month, Gross, median), y=log(Gross))) +
  ...
  xlab("Released Month") +
  ylab("Gross Revenue") +
  ggtitle("Gross Revene by Movie Release Month")
```
