...
for (i in seq(1:nrow(df2))) {
  if (is.na(df2$Gross[i])) {
    ...
    if (is.na(df$Date[i]) | is.na(df2$Released_Year[i])) {
      df2$removal[i] = 0
    }
    else if (!is.na(df2$Date[i]) & df2$Date[i] == df2$Released_Year[i]) {
      df2$removal[i] = 0
    }
  }
    ...
    else if (!is.na(df2$Year[i]) & df2$Year[i] == df2$Released_Year[i]) {
      df2$removal[i] = 0
    }
  }
  else if (is.na(df2$Released_Year[i])) {
    if (!is.na(df2$Date[i]) & df2$Date[i] == df2$Year[i]) {
      df2$removal[i] = 0
    }
  }
  else if (!is.na(df2$Year[i]) & df2$Date[i] == df2$Year[i] & df2$Year[i] == df2$Released_Year[i]) {
    df2$removal[i] = 0
  }
}
